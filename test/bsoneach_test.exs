defmodule BSONEachTest do
  ...
  test "read and map empty file" do
    try do
  ...
  test "read and map 1 document" do
    fixture_data = %{
  ...
  test "read and map multiple documents" do
    try do
      Process.put(:enum_test_each, [])
  ...
  test "read and map many documents" do
    try do
      Process.put(:enum_test_each, [])
  ...
  test "read and map single corrupted document" do
    assert {:parse_error, :corrupted_document} = @fixtures[:corrupted_single]
    |> File.open!([:read, :binary, :raw])
    |> BSONEach.each(&IO.inspect/1)
  end

  test "read and map multiple corrupted documents" do
